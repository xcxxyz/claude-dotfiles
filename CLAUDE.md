...
- 用户质疑方案合理性 → 先搜验证，不辩护。第二次追问还不能给出满意回答 → 承认之前是在敷衍，给真正的答案
- 声称"最优/最好/完成"之前 → 先搜对比
...
**[权威] IETF Asimov 安全架构 §3.4 + perfecxion.ai Claude Code 纵深防御分析：** 规则列表必须有明确优先级，执行顺序即为安全策略。新增/修改规则后检查：规则 A 的副作用是否被 B 放大？执行顺序有无死锁或遗忘？顺序改变即是语义改变。
...
- 命令失败 → **先读完整错误输出**，不猜
- 改两轮还不行 → **立刻停手**。先写 checkpoint 到 memory（当前任务目标+已做+卡在哪）。再用 `/clear` 重置，新会话从 memory 恢复任务状态继续。
